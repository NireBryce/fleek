 ...
 nixpkgs = {
  ...
  programs = { 
    ...
    programs.vscode = { # https://github.com/nix-community/home-manager/blob/master/modules/programs/vscode.nix
      ...
      haskell = {
        enable = false; # haskell integration
        hie = {         # haskell IDE engine
          enable = false; 
          executablePath = "${pkgs.hie-nix.hies}/bin/hie-wrapper"; # default
        };
      };
      ...
      keybindings = [
      # example:  { key = "ctrl+c"; command = "editor.action.clipboardCopyAction"; when = "textInputFocus"; }
